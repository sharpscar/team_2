...
//----갑옷
struct amor  set_amor(
    struct amor a, 
    char a_name[],              // 갑옷이름
    int damage_reduction,       // 데미지감소
    int damage_reduction_rate,  // 데미지감소비율
    int tier,
    int enforce,               
    int price,                  
...
//----물약
struct potion set_potion(
    struct potion p, 
    char p_name[],
    int healing_hp_rate,    //체력회복
    int healing_mp_rate,    //마력회복
    int price,              //가격
    int quantity,           //수량
    int anti_posion,
    int blessing, 
...
//----강화재료
struct refinery_material set_refinery_material(
    struct refinery_material r, 
    char r_name[],      //조각이름
    int refinery_tier,  //
    int is_MTS, 
    int quantity);      //수량
...
//----상태
struct status set_status(struct status s, char s_name[],int lv,int hp,int mp,int atk,int matk,int def,int ac,int cri,int str,int int_,int dex);   
                                        //(이름,             레벨,   체력,  마력,   공격력,  마법공격력, 방어력, 회피율, 치명타율,   힘,     지능,    손재주)
...
struct sword set_sword(struct sword s,char s_name[],int damage,int enforce,int attribute,int price,int cri_rate,int magic_rate)  
{
...
//amor 구조체 반환 함수
struct amor  set_amor(struct amor a, char a_name[],int damage_reduction,int damage_reduction_rate,int tier,int enforce,int price, int additional_damge_rate ,int additional_ac_rate,int additional_cri_rate, int is_debuff)
{
...
// consume 구조체를 반환하는 함수
// 물약 엘릭서 해독제 축복주문서 만병통치약 마을이동주문서 순간이동주문서 들이 있는 consume
struct consume set_consume(struct consume c, char c_name[],int healing_hp_rate,int healing_mp_rate,int price,int quantity,int anti_posion,int is_blessing, int is_all_cure, int is_potal_scroll, int is_teleport_scroll)
{
    strcpy(c.name, c_name);
    c.healing_hp_rate = healing_hp_rate; // 체력회복
    c.healing_mp_rate= healing_mp_rate; // 마나회복(마력회복)
    c.price = price;                    // 가격
    c.quantity = quantity;              // 수량
    c.anti_posion = anti_posion;        // 해독제
    c.is_blessing = is_blessing;        // 축복주문서 (저주상태회복)
    c.is_all_cure = is_all_cure;        // 만병통치약
    c.is_potal_scroll = is_potal_scroll; // 마을이동주문서
    c.is_teleport_scroll =is_teleport_scroll; //순간이동주문서 (원하는곳으로)
...
// refinery_material 구조체를 반환하는 함수
...
// monster구조체 반환하는 함수
struct monster set_monster(struct monster m, char m_name[],int tier_lv_min, int tier_lv_max,int hp_min,int hp_max,int atk_min,int atk_max,int def_min, int def_max,int weak_attr, int strong_attr)// struct amor da,  struct sword ds,struct consume dc,struct refinery_material drm
{
